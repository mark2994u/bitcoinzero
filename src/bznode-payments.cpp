...
//            if (bznodeSync.IsSynced() && !sporkManager.IsSporkActive(SPORK_13_OLD_SUPERBLOCK_FLAG)) {
...
bool IsBlockPayeeValid(const CTransaction &txNew, int nBlockHeight, CAmount blockReward) {
    // we can only check bznode payment /
    const Consensus::Params &consensusParams = Params().GetConsensus();

    if (nBlockHeight < HF_BZNODE_PAYMENT_START) {
        //there is no budget data to use to check anything, let's just accept the longest chain
        if (fDebug) LogPrintf("IsBlockPayeeValid -- bznode isn't start\n");
        return true;
    }
    if (!bznodeSync.IsSynced()) {
        //there is no budget data to use to check anything, let's just accept the longest chain
...
void FillBlockPayments(CMutableTransaction &txNew, int nBlockHeight, CAmount bznodePayment, CTxOut &txoutBznodeRet, std::vector <CTxOut> &voutSuperblockRet) {
...
int CBznodePayments::GetMinBznodePaymentsProto() {
    return sporkManager.IsSporkActive(SPORK_10_BZNODE_PAY_UPDATED_NODES)
           ? MIN_BZNODE_PAYMENT_PROTO_VERSION_2
           : MIN_BZNODE_PAYMENT_PROTO_VERSION_1;
}
...
bool CBznodeBlockPayees::IsTransactionValid(const CTransaction &txNew) {
    ...
    CAmount nBznodePayment = GetBznodePayment(nBlockHeight, txNew.GetValueOut());
...
bool CBznodePaymentVote::IsValid(CNode *pnode, int nValidationHeight, std::string &strError) {
    ...
    if (nBlockHeight >= nValidationHeight) {
        // new votes must comply SPORK_10_BZNODE_PAY_UPDATED_NODES rules
        nMinRequiredProtocol = mnpayments.GetMinBznodePaymentsProto();
    } else {
        // allow non-updated bznodes for old blocks
        nMinRequiredProtocol = MIN_BZNODE_PAYMENT_PROTO_VERSION_1;
    }
