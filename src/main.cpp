...
const string strMessageMagic = "Bitcoinzero Signed Message:\n";
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW,
    ...
    try {
		...
		{
            if (block.vtx[i].IsCoinBase() && nHeight < (forkStartHeight +1) && nHeight > (forkStartHeight + forkHeightRange))
